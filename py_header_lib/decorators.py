...
def validate_func(**schema):
    def decorated(func):
        def validate_required(params):
            for field in schema['required']:
                if field not in params or not params[field]:
                    ...
                    raise BadRequest(message='{field_name} is required'.format(field_name=field_name))
                ...
                for field in schema['enum_type']:
                    if field in params and params[field]:
                        if not schema['enum_type'][field].has_value(params[field]):
                            field_name = field
                            field_name = get_field_name(field_name, field)
                            raise BadRequest(message='{} not isinstance enum'.format(field_name))
                        else:
        ...
        def resource_verb(*args, **kwargs):
            ...
            req_args = {k: v for k, v in kwargs.items() if k in schema['properties'].keys()}
